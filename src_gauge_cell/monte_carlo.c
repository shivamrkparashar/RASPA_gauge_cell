    ...
    for(CurrentCycle=0;CurrentCycle<NumberOfInitializationCycles;CurrentCycle++)
    {
      ...
      for(i=0;i<NumberOfSystems;i++)
      {
        ...
        for(j=0;j<NumberOfSteps;j++)
        {
          ...
          else if(ran<Components[CurrentComponent].ProbabilityGaugeCellSwapMove)  // Shivam
          {
            if(RandomNumber()<0.5) GaugeCellSwapAddMove();
            else GaugeCellSwapRemoveMove();
          }
    ...
    for(CurrentCycle=0;CurrentCycle<NumberOfCycles;CurrentCycle++)
    {
      ...
      for(i=0;i<NumberOfSystems;i++)
      {
        ...
        for(j=0;j<NumberOfSteps;j++)
        {
          ...
          else if(ran<Components[CurrentComponent].ProbabilityGaugeCellSwapMove)
          {
            if(RandomNumber()<0.5) 
            {
              cpu_before=get_cpu_time();
              GaugeCellSwapAddMove();
              cpu_after=get_cpu_time();
              Components[CurrentComponent].CpuTimeGaugeCellSwapMoveInsertion[CurrentSystem]+=(cpu_after-cpu_before);
            }
            else 
            {
              cpu_before=get_cpu_time();
              GaugeCellSwapRemoveMove();
              cpu_after=get_cpu_time();
              Components[CurrentComponent].CpuTimeGaugeCellSwapMoveDeletion[CurrentSystem]+=(cpu_after-cpu_before);
            }
          }
          else if(ran<Components[CurrentComponent].ProbabilityCFSwapLambdaMove)
          {
