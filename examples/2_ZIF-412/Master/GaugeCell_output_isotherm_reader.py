...
for dir_entry in os.listdir(path):
    ...
    Chempot 	  = str(1.0) ; Chempot_STD   = str(1.0)
    Chempot_ex    = str(1.0) ; Chempot_ex_STD= str(1.0)
    ...
    if os.path.isfile(dir_entry_path):
        with open(dir_entry_path, 'r') as my_file:
            for line in my_file:
                ...
                # uncomment for GCMC simulations
                ...
                # These two are only for NVT MC simulations
                if "Average Ideal Gauge Cell chemical potential:" in line:
                    for _ in range(7): my_file.next()
                    line = my_file.next()
                    a,  Chempot, e, Chempot_STD, g = line.split()	
        ...
        results_full.append((Pressure_atm,float(PartialPressure)/1000,float(AAds_cm3_g),float(AAds_cm3_g_STD),
                             float(AAds_Molec_UC),float(AAds_Molec_UC_STD), float(Chempot), float(Chempot_STD), TotalTimeH))
