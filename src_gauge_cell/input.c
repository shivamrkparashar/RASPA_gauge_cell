...
int ReadInput(char *input)
{
  ...
  for (line=strtok_r(tmp, "\n", &tokptr); line; line=strtok_r(NULL, "\n", &tokptr))
  {
    ...
    if(strcasecmp("GaugeCellSize",keyword)==0) sscanf(arguments,"%lf",&GaugeCellSize);
    ...
    if(strcasecmp("SwapProbability",keyword)==0)
      ...
      if(strcasecmp("GaugeCellSwapProbability",keyword)==0)
          if(sscanf(arguments,"%lf",&Components[CurrentComponent].ProbabilityGaugeCellSwapMove))
              if(Components[CurrentComponent].ProbabilityGaugeCellSwapMove>0.0)
              {
                  Components[CurrentComponent].Swapable=TRUE;
                  Swapable=TRUE;
              }
